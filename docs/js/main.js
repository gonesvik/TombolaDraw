...
if (z === '0') {
    if (!seenAtZ0.has(card)) {
    const newText = `${getRandomLetter()} ${getRandomNumber()}`;
    // const newText = `${getRandomNumber()}`;
...
cards.forEach(card => updateCardIfZIndexZero(card));
  ...
  /**
function evalShortcutX(event) {
    let registrationMode = document.querySelector('#read-only').style.display == 'none';
    let drawingMode = document.querySelector('#winner-ticket').style.display == 'flex';
    let startResetting = document.querySelector('#reset').style.display == 'block';

    switch (event.keyCode) {
        case 13: // <Enter> or <Space>
        case 32: {
            if (drawingMode) drawTicket();  // draw
            break;
        }
        case 33: { // <PageUp>
            if (!drawingMode) break;
            if (gv_drawHistory.length === 0) regretRegistration();  // regret
            else
                startReview();
            break;
        }
        case 34: { // <PageDown>
            if (gv_drawHistory.length === 0) registerTickets();  // ready
            else endReview();  // close
            break;
        } 
        case 37: { // <ArrowLeft>
            if (!drawingMode) break;
            if (gv_historyIndex > 0) traverseHistory(-1);  // prev
            break;
        } 
        case 39: { // <ArrowRight>
            if (!drawingMode) break;
            if (gv_historyIndex < gv_drawHistory.length - 1) traverseHistory(1);  // next
            break;
        }
        case 45: { // <Insert>
            if (registrationMode) addRecord();  // insert
            break;
        }
        case 78: { // character N
            if (startResetting) document.querySelector('#reset').style.display = 'none';
            break;
        } 
        case 89: { // charater Y
            if (startResetting) resetApp();
            break;
        }
    }
}
